...
ID3D12RootSignature *CScene::CreateGraphicsRootSignature(ID3D12Device *pd3dDevice)
{
	...
	pd3dDescriptorRanges[i].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_CBV;
	pd3dDescriptorRanges[i].NumDescriptors = 1;
	pd3dDescriptorRanges[i].BaseShaderRegister = 2; // c2: GameObject
	pd3dDescriptorRanges[i].RegisterSpace = 0;
	pd3dDescriptorRanges[i++].OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;
	
	pd3dDescriptorRanges[i].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV;
	pd3dDescriptorRanges[i].NumDescriptors = 1;
	pd3dDescriptorRanges[i].BaseShaderRegister = 6; //t6: gtxtAlbedoTexture
	pd3dDescriptorRanges[i].RegisterSpace = 0;
	pd3dDescriptorRanges[i++].OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;

	pd3dDescriptorRanges[i].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV;
	pd3dDescriptorRanges[i].NumDescriptors = 1;
	pd3dDescriptorRanges[i].BaseShaderRegister = 7; //t7: gtxtSpecularTexture
	pd3dDescriptorRanges[i].RegisterSpace = 0;
	pd3dDescriptorRanges[i++].OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;

	pd3dDescriptorRanges[i].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV;
	pd3dDescriptorRanges[i].NumDescriptors = 1;
	pd3dDescriptorRanges[i].BaseShaderRegister = 8; //t8: gtxtNormalTexture
	pd3dDescriptorRanges[i].RegisterSpace = 0;
	pd3dDescriptorRanges[i++].OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;

	pd3dDescriptorRanges[i].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV;
	pd3dDescriptorRanges[i].NumDescriptors = 1;
	pd3dDescriptorRanges[i].BaseShaderRegister = 9; //t9: gtxtMetallicTexture
	pd3dDescriptorRanges[i].RegisterSpace = 0;
	pd3dDescriptorRanges[i++].OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;

	pd3dDescriptorRanges[i].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV;
	pd3dDescriptorRanges[i].NumDescriptors = 1;
	pd3dDescriptorRanges[i].BaseShaderRegister = 10; //t10: gtxtEmissionTexture
	pd3dDescriptorRanges[i].RegisterSpace = 0;
	pd3dDescriptorRanges[i++].OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;

	pd3dDescriptorRanges[i].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV;
	pd3dDescriptorRanges[i].NumDescriptors = 1;
	pd3dDescriptorRanges[i].BaseShaderRegister = 11; //t11: gtxtEmissionTexture
	pd3dDescriptorRanges[i].RegisterSpace = 0;
	pd3dDescriptorRanges[i++].OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;

	pd3dDescriptorRanges[i].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV;
	pd3dDescriptorRanges[i].NumDescriptors = 1;
	pd3dDescriptorRanges[i].BaseShaderRegister = 12; //t12: gtxtEmissionTexture
	pd3dDescriptorRanges[i].RegisterSpace = 0;
	pd3dDescriptorRanges[i++].OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;

	pd3dDescriptorRanges[i].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV;
	pd3dDescriptorRanges[i].NumDescriptors = 1;
	pd3dDescriptorRanges[i].BaseShaderRegister = 13; //t13: gtxtSkyBoxTexture
	pd3dDescriptorRanges[i].RegisterSpace = 0;
	pd3dDescriptorRanges[i++].OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;

	pd3dDescriptorRanges[i].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV;
	pd3dDescriptorRanges[i].NumDescriptors = 1;
	pd3dDescriptorRanges[i].BaseShaderRegister = 1; //t1: gtxtTerrainBaseTexture
	pd3dDescriptorRanges[i].RegisterSpace = 0;
	pd3dDescriptorRanges[i++].OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;

	pd3dDescriptorRanges[i].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV;
	pd3dDescriptorRanges[i].NumDescriptors = 1;
	pd3dDescriptorRanges[i].BaseShaderRegister = 2; //t2: gtxtTerrainDetailTexture
	pd3dDescriptorRanges[i].RegisterSpace = 0;
	pd3dDescriptorRanges[i++].OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;

	pd3dDescriptorRanges[i].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV;
	pd3dDescriptorRanges[i].NumDescriptors = 1;
	pd3dDescriptorRanges[i].BaseShaderRegister = 14; //t14: Texture
	pd3dDescriptorRanges[i].RegisterSpace = 0;
	pd3dDescriptorRanges[i++].OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;
	...
	i = 0;
	pd3dRootParameters[i].ParameterType = D3D12_ROOT_PARAMETER_TYPE_CBV;
	pd3dRootParameters[i].Descriptor.ShaderRegister = 1; //Camera
	pd3dRootParameters[i].Descriptor.RegisterSpace = 0;
	pd3dRootParameters[i++].ShaderVisibility = D3D12_SHADER_VISIBILITY_ALL;

	pd3dRootParameters[i].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;
	pd3dRootParameters[i].DescriptorTable.NumDescriptorRanges = 1;
	pd3dRootParameters[i].DescriptorTable.pDescriptorRanges = &pd3dDescriptorRanges[0];
	pd3dRootParameters[i++].ShaderVisibility = D3D12_SHADER_VISIBILITY_ALL;

	pd3dRootParameters[i].ParameterType = D3D12_ROOT_PARAMETER_TYPE_CBV;
	pd3dRootParameters[i].Descriptor.ShaderRegister = 4; //Lights
	pd3dRootParameters[i].Descriptor.RegisterSpace = 0;
	pd3dRootParameters[i++].ShaderVisibility = D3D12_SHADER_VISIBILITY_ALL;

	pd3dRootParameters[i].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;
	pd3dRootParameters[i].DescriptorTable.NumDescriptorRanges = 1;
	pd3dRootParameters[i].DescriptorTable.pDescriptorRanges = &(pd3dDescriptorRanges[1]);
	pd3dRootParameters[i++].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;

	pd3dRootParameters[i].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;
	pd3dRootParameters[i].DescriptorTable.NumDescriptorRanges = 1;
	pd3dRootParameters[i].DescriptorTable.pDescriptorRanges = &(pd3dDescriptorRanges[2]);
	pd3dRootParameters[i++].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;

	pd3dRootParameters[i].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;
	pd3dRootParameters[i].DescriptorTable.NumDescriptorRanges = 1;
	pd3dRootParameters[i].DescriptorTable.pDescriptorRanges = &(pd3dDescriptorRanges[3]);
	pd3dRootParameters[i++].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;

	pd3dRootParameters[i].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;
	pd3dRootParameters[i].DescriptorTable.NumDescriptorRanges = 1;
	pd3dRootParameters[i].DescriptorTable.pDescriptorRanges = &(pd3dDescriptorRanges[4]);
	pd3dRootParameters[i++].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;

	pd3dRootParameters[i].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;
	pd3dRootParameters[i].DescriptorTable.NumDescriptorRanges = 1;
	pd3dRootParameters[i].DescriptorTable.pDescriptorRanges = &(pd3dDescriptorRanges[5]);
	pd3dRootParameters[i++].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;

	pd3dRootParameters[i].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;
	pd3dRootParameters[i].DescriptorTable.NumDescriptorRanges = 1;
	pd3dRootParameters[i].DescriptorTable.pDescriptorRanges = &(pd3dDescriptorRanges[6]);
	pd3dRootParameters[i++].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;

	pd3dRootParameters[i].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;
	pd3dRootParameters[i].DescriptorTable.NumDescriptorRanges = 1;
	pd3dRootParameters[i].DescriptorTable.pDescriptorRanges = &(pd3dDescriptorRanges[7]);
	pd3dRootParameters[i++].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;

	pd3dRootParameters[i].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;
	pd3dRootParameters[i].DescriptorTable.NumDescriptorRanges = 1;
	pd3dRootParameters[i].DescriptorTable.pDescriptorRanges = &(pd3dDescriptorRanges[8]);
	pd3dRootParameters[i++].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;

	pd3dRootParameters[i].ParameterType = D3D12_ROOT_PARAMETER_TYPE_CBV;
	pd3dRootParameters[i].Descriptor.ShaderRegister = 7; //Skinned Bone Offsets
	pd3dRootParameters[i].Descriptor.RegisterSpace = 0;
	pd3dRootParameters[i++].ShaderVisibility = D3D12_SHADER_VISIBILITY_VERTEX;

	pd3dRootParameters[i].ParameterType = D3D12_ROOT_PARAMETER_TYPE_CBV;
	pd3dRootParameters[i].Descriptor.ShaderRegister = 8; //Skinned Bone Transforms
	pd3dRootParameters[i].Descriptor.RegisterSpace = 0;
	pd3dRootParameters[i++].ShaderVisibility = D3D12_SHADER_VISIBILITY_VERTEX;

	pd3dRootParameters[i].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;
	pd3dRootParameters[i].DescriptorTable.NumDescriptorRanges = 1;
	pd3dRootParameters[i].DescriptorTable.pDescriptorRanges = &(pd3dDescriptorRanges[9]);
	pd3dRootParameters[i++].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;

	pd3dRootParameters[i].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;
	pd3dRootParameters[i].DescriptorTable.NumDescriptorRanges = 1;
	pd3dRootParameters[i].DescriptorTable.pDescriptorRanges = &(pd3dDescriptorRanges[10]);
	pd3dRootParameters[i++].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;

	pd3dRootParameters[i].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;
	pd3dRootParameters[i].DescriptorTable.NumDescriptorRanges = 1;
	pd3dRootParameters[i].DescriptorTable.pDescriptorRanges = &(pd3dDescriptorRanges[11]);
	pd3dRootParameters[i++].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;
