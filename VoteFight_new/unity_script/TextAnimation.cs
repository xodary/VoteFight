...
public class TextAnimation : MonoBehaviour
{
   ...
   void WriteString(string strToWrite)
    ...
    {
        WriteLineString(nLevel, "<Animation>");
#if _WITH_SKINNED_BONES_ANIMATION
        WriteSkinnedMeshFrameNames(nLevel+1, "<Name> ");
#else
        WriteFrameNames(nLevel+1, "<Name> ");
#endif
        for (int j = 0; j < m_raAnimationClips.Length; j++)
        {
            ...
            WriteLineString(nLevel + 1, "<FrameRate> " + (int)m_raAnimationClips[j].frameRate);
            WriteLineString(nLevel + 1, "<FrameCount> " + nKeyFrames);
            WriteLineString(nLevel + 1, "<Duration> " + m_raAnimationClips[j].averageDuration);
            WriteLineString(nLevel + 1, "<SkinnedMeshes> 1");
            float fFrameRate = (1.0f / nFramesPerSec), fKeyFrameTime = 0.0f;
            WriteLineString(nLevel + 1, "<ElapsedTime> " + fKeyFrameTime);
            for (int k = 0; k < nKeyFrames; k++)
            {
                WriteLineString(nLevel + 1, "<BoneTransformMatrix> " + GetBoneCount(m_raAnimationClips[j]));
...
#else
                WriteAnimationTransforms(nLevel+2,"");
#endif
    ...
    // AnimationClip에서 사용된 모든 본의 수를 가져오는 함수
    int GetBoneCount(AnimationClip clip)
    {
        // 애니메이션 클립에 사용된 모든 본 가져오기
    ...
    void Start()
    {
        ...
        {
    		WriteLineString("<Animations> "+ m_raAnimationClips.Length);
