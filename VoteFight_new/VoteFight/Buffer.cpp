	...
	, mCBVMappedData(nullptr)
{
	...
	CreateResources(descriptorManager);
...
void CBuffer::CreateResources(DESC::DescriptorHeapManager* descriptorManager)
{
	...
	if (mDescription.mDescriptorType & DescriptorType::SRV)
	{
		...
		else
		{
			...
			if (mDescription.mDescriptorType & CBuffer::DescriptorType::Structured)
			{
				srvDesc.Buffer.StructureByteStride = mDescription.mElementSize;
			}
			else if (mDescription.mDescriptorType & CBuffer::DescriptorType::Raw)
			{
