...
void GraphicsPSO::Finalize(ID3D12Device* device)
{
    ...
    if (firstCompile)
    {
        DX::ThrowIfFailed(device->CreateGraphicsPipelineState(&m_PSODesc, IID_PPV_ARGS(&m_PSO)));
...
void ComputePSO::Finalize(ID3D12Device* device)
{
    ...
    if (firstCompile)
    {
        DX::ThrowIfFailed(device->CreateComputePipelineState(&m_PSODesc, IID_PPV_ARGS(&m_PSO)));
