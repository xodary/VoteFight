...
void CGameScene::ImGuiRenderMiniMap()
{
	...
	for (int i = static_cast<int>(GROUP_TYPE::STRUCTURE); i <= static_cast<int>(GROUP_TYPE::GROUND_ITEM); ++i) {
		...
		for (const auto& objectsPair : objects) {
			...
			// È®´ë ¸ðµåÀÏ ¶§ Áß½É °´Ã¼ ±âÁØ ÁÂÇ¥ º¯È¯
			float mapX, mapY;
			if (m_miniMap) {
				mapX = (npcPos3D.x - 200.f) / 600.0f * windowSize.x;
				mapY = (npcPos3D.z - 200.f) / 600.0f * windowSize.y; // YÃà ¹ÝÀü
			}
			else {
				mapX = (npcPos3D.x - focusPos3D.x) / 600.0f * windowSize.x * scaleFactor;
				mapY = (npcPos3D.z - focusPos3D.z) / 600.0f * windowSize.y * scaleFactor; // YÃà ¹ÝÀü
			}
...
void CGameScene::SetLightVersion(int value)
{
	...
	if (value % 2 != 0)
	{
		m_mappedGameScene->m_lights[1].m_xmf4Ambient = XMFLOAT4(0.6f, 0.6f, 0.6f, 1.0f); // ¹à°í µû¶æÇÑ »ö»ó
		m_mappedGameScene->m_lights[1].m_xmf4Diffuse = XMFLOAT4(1.0f, 1.0f, 0.9f, 1.0f); // °­ÇÏ°í ¼±¸íÇÑ Èò»ö
		m_mappedGameScene->m_lights[1].m_xmf4Specular = XMFLOAT4(0.9f, 0.9f, 0.8f, 1.0f); // ³ôÀº °­µµÀÇ Èò»ö
		m_mappedGameScene->m_lights[1].m_isActive = true;
		m_mappedGameScene->m_lights[1].m_shadowMapping = false;
		m_mappedGameScene->m_lights[1].m_type = static_cast<int>(LIGHT_TYPE::DIRECTIONAL);
		m_mappedGameScene->m_lights[1].m_position = XMFLOAT3(0.0f, 100.0f, 0.0f); // ÅÂ¾çÀÌ ÇÏ´Ã¿¡ ÀÖ´Â °ÍÃ³·³ À§¿¡¼­ ¾Æ·¡·Î ºñÃß´Â ¹æÇâ
	...
	{
		m_mappedGameScene->m_lights[1].m_xmf4Ambient = XMFLOAT4(0.1f, 0.1f, 0.2f, 1.0f); // ¾îµÓ°í Â÷°¡¿î »ö»ó
		m_mappedGameScene->m_lights[1].m_xmf4Diffuse = XMFLOAT4(0.2f, 0.2f, 0.4f, 1.0f); // ¾àÇÑ Çª¸¥ºû
		m_mappedGameScene->m_lights[1].m_xmf4Specular = XMFLOAT4(0.3f, 0.3f, 0.5f, 1.0f); // ³·Àº °­µµÀÇ Èò»ö ¶Ç´Â Çª¸¥»ö
		m_mappedGameScene->m_lights[1].m_isActive = true;
		m_mappedGameScene->m_lights[1].m_shadowMapping = false;
		m_mappedGameScene->m_lights[1].m_type = static_cast<int>(LIGHT_TYPE::DIRECTIONAL);
		m_mappedGameScene->m_lights[1].m_position = XMFLOAT3(0.0f, 10.0f, -10.0f); // ´ÞºûÀÌ³ª µµ½Ã ºÒºûÀ» ¹Ý¿µÇÏ¿© ´Ù¾çÇÑ ¹æÇâ
