...
VS_STANDARD_OUTPUT VS_OCEAN_GBuffer(VS_STANDARD_INPUT input)
{
    VS_STANDARD_OUTPUT output;
   ...
   // ¹Ù´ÙÀÇ ¿òÁ÷ÀÓÀ» À§ÇÑ ³ëÀÌÁî ±â¹Ý Y ÁÂÇ¥ º¯Çü
    float waveHeight = 0.8f; // ¹°°áÀÇ ³ôÀÌ
    float waveFrequency = 0.1f; // ¹°°áÀÇ ÁÖ±â
    float waveSpeed = 2.8f; // ¹°°áÀÇ ¼Óµµ
    
    // Áß½ÉÁ¡(¼¶ÀÇ À§Ä¡)À» ±âÁØÀ¸·Î ÆÄµµ È¿°ú Àû¿ë
    float3 center = float3(0.0, 0.0, 0.0);
    float3 direction = normalize(center - input.position);

    // ¹æÇâ º¤ÅÍ¸¦ ±â¹ÝÀ¸·Î ³ëÀÌÁî ÀÔ·ÂÀ» °è»ê
    float distanceToCenter = length(center - pW.xyz);
    float2 noiseInput = float2(distanceToCenter * waveFrequency, gfTotalTime * waveSpeed);

    // ÆÄµµ ¹æÇâÀ» Áß½ÉÀ» ÇâÇØ º¯Çü
