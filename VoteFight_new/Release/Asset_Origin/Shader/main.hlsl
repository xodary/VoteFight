...
float4 PS_Main(VS_STANDARD_OUTPUT input) : SV_TARGET
{
  ...
  // cColor = (lerp(cColor, cIllumination, 0.5f));
    ...
    // Adjust UV coordinates to position the donut at the left side of the screen
    float2 uv = input.uv * 0.3; // Scale down to make the donut smaller
    uv += float2(-0.35, 0.35); // Move to the left side of the screen

    float2 center = float2(0.15, 0.5); // New center for the small donut
    uv = uv - center;
    float angle = atan2(uv.y, uv.x) + 3.14159265358979323846; // Shift to [0, 2PI]
    float dist = length(uv);

    // Determine the section based on the angle
    float ratio = angle / (2.0 * 3.14159265358979323846);
    float4 color = float4(0, 0, 0, 1);

    float4 sectionRatios = float4(0.25, 0.5, 0.25, 0.0);
    if (ratio < sectionRatios.x)
    {
        color = float4(1, 0, 0, 1); // Red for the first section
    }
    else if (ratio < sectionRatios.x + sectionRatios.y)
    {
        color = float4(0, 1, 0, 1); // Green for the second section
    }
    else
    {
        color = float4(0, 0, 1, 1); // Blue for the third section
    }

    // Create the donut shape: Inner radius should be > 0, and outer radius <= 1
    float innerRadius = 0.2;
    float outerRadius = 0.3;

    if (dist < innerRadius || dist > outerRadius)
    {
        color = float4(0, 0, 0, 0); // Make the outside of the donut transparent
    }
